// Server Component — hover handled by Tailwind CSS, no client JS needed
...
export default function Footer() {
  ...
  return (
    <footer className="border-t border-white/6 py-10 bg-[rgba(10,10,15,0.8)]">
      <div
        className="container-custom flex items-center justify-between flex-wrap gap-5"
      >
        {/* Logo & copyright */}
        <div className="flex items-center gap-3">
          <div
            className="w-8 h-8 rounded-[8px] flex items-center justify-center bg-(--gradient-primary)"
          >
          ...
          <div>
            ...
            <div className="text-[12px] flex items-center gap-1 text-(--text-muted)">
              © {year} · Made with{" "}
              <Heart size={11} className="text-[#f472b6]" /> in Hà Nội
            </div>
          </div>
        </div>
        ...
        <div className="flex gap-3">
          {socialLinks.map((link) => (
            <a
              ...
              className="
                w-9 h-9 flex items-center justify-center no-underline
                rounded-[8px]
